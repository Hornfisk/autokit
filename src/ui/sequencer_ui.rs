...
use crate::engine::sequencer::{ConditionTrig, NUM_PATTERNS, NUM_STEPS};
use crate::ui::knob;
use crate::ui::theme;
...
pub fn draw_sequencer_view(
    ...
    let (grid_actions, grid_layout) =
        draw_grid(ui, display, available_height, view_state, tooltips_on);
    actions.extend(grid_actions);
    let _grid_left = grid_layout.grid_left_x;
    ...
    ui.horizontal(|ui| {
        ...
        // SWING horizontal bar — drag left/right, double-click to reset.
        {
            ...
            let (rect, response) = ui
                .allocate_exact_size(Vec2::new(swing_bar_w, bar_h), egui::Sense::click_and_drag());
        ...
        if let Some((lane_idx, step_idx)) = view_state.selected {
            if lane_idx < display.lanes.len() {
                ...
                actions.extend(draw_param_controls(
                    ui,
                    lane_idx,
                    step_idx,
                    step,
...
fn draw_grid(
    ...
    let had_active_drag = view_state.drag_paint.is_some() || view_state.drag_origin.is_some();
        ...
        if let Some(pos) = ui.input(|i| i.pointer.hover_pos()) {
            ...
            if let Some((origin_lane, origin_step)) = view_state.drag_origin {
                if !view_state.drag_is_velocity && view_state.drag_paint.is_none() {
                    ...
                    if moved_horizontal {
                        ...
                        actions.push(SeqAction::SetStepEnabled {
                            lane: origin_lane,
                            step: origin_step,
                            enabled: new_state,
                        });
            ...
            if view_state.drag_is_velocity {
                if let Some((origin_lane, origin_step)) = view_state.drag_origin {
                    ...
                    actions.push(SeqAction::SetStepVelocity {
                        lane: origin_lane,
                        step: origin_step,
                        value: new_vel,
                    });
            ...
            if let Some(enable) = view_state.drag_paint {
                for &(lane_idx, step_idx, rect) in &view_state.cell_rects {
                    if rect.contains(pos)
                        && !view_state.drag_visited.contains(&(lane_idx, step_idx))
                    {
                        view_state.drag_visited.insert((lane_idx, step_idx));
                        actions.push(SeqAction::SetStepEnabled {
                            lane: lane_idx,
...
fn draw_param_controls(
    ...
    let st_resp = knob::knob(
        ui,
        egui::Id::new(("seq_start", lane_idx)),
        &mut st,
        0.0,
        0.99,
        0.0,
    ...
    if st_resp.changed {
        actions.push(SeqAction::SetPadStart {
            lane: lane_idx,
            value: st,
        });
    }
    ...
    if en_resp.changed {
        actions.push(SeqAction::SetPadEnd {
            lane: lane_idx,
            value: en,
        });
    }
    ...
    ui.painter().vline(
        ui.cursor().left(),
        ui.cursor().top()..=ui.cursor().top() + ksz,
    ...
    let pan_color = if step.pan.is_some() {
        theme::PLOCK_DOT
    } else {
        gray
    };
    let pan_resp = knob::knob(
        ...
        |v| {
            if v.abs() < 0.01 {
                ...
                format!("L{}", (-v * 100.0) as u8)
            } else {
                format!("R{}", (v * 100.0) as u8)
            }
        },
        pan_color,
        ksz,
    ...
    if pan_resp.changed {
        actions.push(SeqAction::SetStepPan {
            lane: lane_idx,
            step: step_idx,
            value: Some(pan),
        });
    ...
    let pitch_color = if step.pitch.is_some() {
        theme::PLOCK_DOT
    } else {
        gray
    };
    ...
    if pitch_resp.changed {
        actions.push(SeqAction::SetStepPitch {
            lane: lane_idx,
            step: step_idx,
            value: Some(pitch),
        });
    ...
    if prob_resp.changed {
        actions.push(SeqAction::SetStepProbability {
            lane: lane_idx,
            step: step_idx,
            value: prob,
        });
    ...
    ui.vertical(|ui| {
        ...
        let cond_color = if step.condition != ConditionTrig::Always {
            theme::COND_TEXT
        } else {
            theme::TEXT_DIM
        };
            ...
            .show_ui(ui, |ui| {
                for &cond in ConditionTrig::CYCLE {
                    ...
                    if resp.clicked() && !is_active {
                        actions.push(SeqAction::SetStepCondition {
                            lane: lane_idx,
                            step: step_idx,
                            condition: cond,
                        });
    ...
    let rvb_color = if step.fx_rvb.is_some() {
        theme::FX_REVERB
    } else {
        gray
    };
    let rvb_resp = knob::knob(
        ui,
        egui::Id::new(("seq_rvb_lock", lane_idx, step_idx)),
        &mut rvb,
        0.0,
        1.0,
        0.0,
    ...
    if rvb_resp.changed {
        actions.push(SeqAction::SetStepReverbLock {
            lane: lane_idx,
            step: step_idx,
            value: Some(rvb),
        });
    ...
    let dly_color = if step.fx_dly.is_some() {
        theme::FX_DELAY
    } else {
        gray
    };
    let dly_resp = knob::knob(
        ui,
        egui::Id::new(("seq_dly_lock", lane_idx, step_idx)),
        &mut dly,
        0.0,
        1.0,
        0.0,
    ...
    if dly_resp.changed {
        actions.push(SeqAction::SetStepDelayLock {
            lane: lane_idx,
            step: step_idx,
            value: Some(dly),
        });
    ...
    ui.vertical(|ui| {
        ...
        if resp.clicked() {
            ...
            actions.push(SeqAction::SetStepFilterLock {
                lane: lane_idx,
                step: step_idx,
                value: next,
            });
...
fn draw_bottom_bar(
    ...
    ui.vertical(|ui| {
        ...
        ui.horizontal(|ui| {
            ...
            let play_label = if display.playing {
                "\u{25A0} STOP"
            } else {
                "\u{25B6} PLAY"
            };
            let play_color = if display.playing {
                Color32::BLACK
            } else {
                theme::ACCENT
            };
            let play_bg = if display.playing {
                theme::ACCENT
            } else {
                theme::STEP_BG
            };
            ...
            let fill_bg = if display.fill_active {
                theme::FILL_PURPLE
            } else {
                Color32::TRANSPARENT
            };
            let fill_text_color = if display.fill_active {
                Color32::WHITE
            } else {
                theme::FILL_PURPLE
            };
